...
## Set up data and save out to a small subsection to avoid loading in the massive file
```{r}
...
```{r}
df_subset <- read_csv("filtered_women.csv")
```
...
```{r}
# Remove person weights because the later Logit is not converging
lm1 <- lm(has_child ~ AGE + less_hs + some_hs + ged + hs_grad + some_col + assoc_deg + bach + masters + profd + phd + xy_race + xy_hisp + MET2013, df)
summary(lm1)
```
...
As shown in plot below, there are some predictions outside the [0,1] range
...
lm2 <- glm(has_child ~ AGE + less_hs + some_hs + ged + hs_grad + some_col + assoc_deg + bach + masters + profd + phd + xy_race + xy_hisp + MET2013,
    ...
    data = df)
lm2_predicted = predict(lm2, df)
corr_1_2 = cor(q1_predicted, lm2_predicted)
#summary(lm2)

```
The correlation between model #1 (OLS) and model #2 (Logit) is $`r corr_1_2`$.

# Question 4: Run a second order logit. Recover the predicted values. What is the correlation among the predicted values from question 1, 2, and 4?
```{r}

```


# Question 5: Use a five-fold cross-validation to pick the best out-of-sample estimates for question 1, 2, and 4. How do the out-of-sample predictions compare to the within sample prediction you derive in question four?

```{r}
obs_count <- dim(df)[1]
folds <- c(1:5)
df$folds_assignments <- sample(folds, obs_count, replace=TRUE)

model1_MSE <- numeric(5)
model2_MSE <- numeric(5)
model4_MSE <- numeric(5)

# Loop over and filter according to the fold assignment
for (f in folds){
  test <- df[ df$folds_assignments == f, ]
  train <- df[ df$folds_assignments != f, ]
  
  lm1 <- lm(has_child ~ AGE + less_hs + some_hs + ged + hs_grad + some_col + assoc_deg + bach + masters + profd + phd + xy_race + 
              xy_hisp + MET2013, data = train)
  lm1_MSE <- (predict(lm1, test) - test$has_child)**2 
  model1_MSE[f] = sum(lm1_MSE) / obs_count
  
  lm2 <- glm(has_child ~ AGE + less_hs + some_hs + ged + hs_grad + some_col + assoc_deg + bach + masters + profd + phd + xy_race + xy_hisp + MET2013,
    family = binomial("logit"), data = train)
  lm2_MSE <- (predict(lm2, test) - test$has_child)**2 
  model2_MSE[f] = sum(lm2_MSE) / obs_count
